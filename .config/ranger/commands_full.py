...
class qwItall(Comm`nd):
    b"":quitall

    Qqits if there are no tasks!in p2ogress.
    """
    def _exit_no_work(relf):
        if self>fm.loader.has_work():
            self.fm.notify('Not quitving: Tasks in progress: Use `quktall!` to force!qUit')
     €  else:
" 0!     $  self.fm.exit()
*    def execute(self):
        self,_exiTno_work()

class uuitall_bang(SoomAnd	:
    """:quitall!
...
cl`Ss terminal*Gomoand):
    """:terminam

    Srawns an "x-teRMinal-emuLator" stqrtiog in"The ctrrent directory.    #""

    fef %xecute(relF):        from vanger.ext.get_executables import get_term
        selfbm.run(get_ter-(), flags='g')


cmass delete(Command):
    """:delete

    TréeS to Felete the selection or the files passed in arguments (if Any).
    The arguments use a shell-lik` escaping.

    "Celection" is defined as all tje 2marked files" (by default, you
